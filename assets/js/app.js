...
(() => {
  ...
  const ICONS = {
    ...
    smooth: '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8c2 3 4-3 6 0s4-3 6 0 4-3 6 0"/><path d="M3 16c2 3 4-3 6 0s4-3 6 0 4-3 6 0"/></svg>',
  };
  ...
  el.brandHomeBtn.addEventListener("click", () => { closeCamera(); showScreen("category"); });
  ...
  function loadImage(dataUrl, initialSettings) {
    ...
    img.onerror = () => {
      // Certains navigateurs (la plupart hors Safari) ne savent pas décoder
      // le HEIC nativement : on prévient plutôt que de laisser l'aperçu
      // dans un état incohérent.
      alert("Impossible d'ouvrir cette image. Si c'est un fichier HEIC, essayez de l'exporter en JPG (certains navigateurs ne savent pas encore lire le HEIC).");
    };
    img.src = dataUrl;
  }
  ...
  // ---------------------------------------------------------------------
  // Calcule l'image finale à pleine résolution (cadrage + réglages) et la
  // renvoie sous forme de Blob PNG. Partagé par les deux boutons d'export.
  ...
  function setExportBusy(busy) {
    el.canvasLoading.classList.toggle("active", busy);
    el.downloadBtn.disabled = busy;
    el.galleryBtn.disabled = busy;
  }
  ...
  // "Télécharger" : sur les navigateurs qui le permettent (Chrome / Edge
  // ordinateur), ouvre le vrai sélecteur natif "Enregistrer sous" pour
  // choisir l'emplacement et le nom du fichier. Ailleurs, retombe sur le
  // téléchargement classique du navigateur (dossier de téléchargements).
  el.downloadBtn.addEventListener("click", async () => {
    ...
    if (window.showSaveFilePicker) {
      try {
        ...
        // Fenêtre annulée par l'utilisateur : on n'insiste pas.
        if (err && err.name === "AbortError") { setExportBusy(false); return; }
        // Sinon (API indisponible dans ce contexte) : téléchargement classique.
      }
    }
  ...
  // "Enregistrer dans la galerie" : ouvre la feuille de partage native
  // (mobile principalement), avec l'option "Enregistrer l'image" /
  // "Enregistrer dans Photos". Si le navigateur ne le permet pas pour ce
  // fichier (la plupart des ordinateurs), on retombe sur un téléchargement
  // classique plutôt que de laisser le bouton sans effet.
  el.galleryBtn.addEventListener("click", async () => {
    if (!state.fullCanvas) return;
    setExportBusy(true);
    ...
    if (canShareFile) {
      try {
        ...
        if (err && err.name === "AbortError") { setExportBusy(false); return; }
        // Sinon : le partage a échoué pour une autre raison, on retombe
        // sur le téléchargement classique ci-dessous.
      }
    }
