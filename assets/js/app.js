...
(() => {
    ...
    SLIDER_DEFS.forEach(def => {
      ...
      row.innerHTML = `
        <div class="slider-head">
          ...
          <span class="slider-value" data-value-for="${def.key}">${value}</span>
        </div>
  ...
  function onSliderInput(e) {
    ...
    updateSliderFill(input);
    const valueLabel = el.sliderContainer.querySelector(`[data-value-for="${key}"]`);
  ...
  function syncSlidersFromSettings() {
    el.sliderContainer.querySelectorAll("input[type=range]").forEach(input => {
      ...
      updateSliderFill(input);
      const valueLabel = el.sliderContainer.querySelector(`[data-value-for="${key}"]`);
      if (valueLabel) valueLabel.textContent = state.settings[key];
    });
  }
