/* ==========================================================================
   ...
   6. Clarté (contraste local, fait ressortir le relief et les textures)
   7. Lissage des couleurs (adoucit la chrominance, garde la luminance nette)
   8. Netteté (convolution)  9. Halation (bloom sur les hautes lumières)
   10. Vignette  11. Grain
   ========================================================================== */
...
const Engine = (() => {
  ...
  function applyAll(imageData, settings) {
    ...
    // 6. Clarté : contraste local sur les tons moyens, fait ressortir le
    // relief (montagnes, roche, textures de terrain) sans toucher aux
    // ombres et hautes lumières comme le ferait un contraste global.
    if (settings.clarity > 0) {
      clarity(result, settings.clarity / 100);
    }

    // 7. Lissage des couleurs : adoucit la chrominance (les teintes) en
    // préservant exactement la luminance d'origine, donc sans jamais flouter
    // les contours ni le relief. C'est ce qui donne ce rendu "poli", propre
    // aux logiciels d'édition IA type Luminar : les couleurs se fondent
    // délicatement pendant que les textures (renforcées par la clarté)
    // restent nettes.
    if (settings.chromaSmooth > 0) {
      chromaSmooth(result, settings.chromaSmooth / 100);
    }

    // 8. Netteté (convolution) (passe séparée car nécessite les pixels voisins)
    if (settings.sharpen > 0) {
      result = sharpen(result, settings.sharpen / 100);
    }

    // 8. Halation (bloom chaleureux) sur les hautes lumières, caractéristique
    ...
    // 10. Grain, appliqué en dernier, après tous les autres traitements
    if (settings.grain > 0) {
  ...
  /**
   * Clarté : contraste local façon "clarity" (Lightroom). On isole la
   * luminance, on la floute avec un grand rayon (proportionnel à la taille
   * de l'image, donc un rendu cohérent quelle que soit la résolution), puis
   * on réinjecte l'écart entre l'original et le flou (le détail "moyenne
   * fréquence") sur les trois canaux. L'effet est atténué dans les ombres
   * et les hautes lumières pour éviter les halos disgracieux et rester
   * concentré sur les tons moyens, là où vivent le relief et les textures.
   */
  function clarity(imageData, amount) {
    ...
    const blurred = lum.slice();
    const radius = Math.max(2, Math.round(Math.min(width, height) * 0.025));
    boxBlur(blurred, width, height, radius, 2);

    const strength = amount * 0.85;
    for (let p = 0, i = 0; p < n; p++, i += 4) {
      const highPass = lum[p] - blurred[p];
      const t = lum[p] / 255;
      const midtoneWeight = Math.max(0.35, 1 - (2 * t - 1) * (2 * t - 1));
      const add = highPass * strength * midtoneWeight;
  ...
  /**
   * Lissage des couleurs : sépare la luminance (Y) de la chrominance
   * (Cb/Cr), floute uniquement la chrominance avec un rayon proportionnel
   * à l'image, puis recompose avec la luminance d'origine inchangée. Le
   * résultat adoucit les variations de teinte (grain de couleur, mouchetures)
   * sans jamais flouter un seul contour, puisque toute la netteté d'une
   * image vit dans le canal de luminance. C'est ce mélange "couleurs
   * fondues / contours nets" qui donne un rendu poli façon retouche IA.
   */
  function chromaSmooth(imageData, amount) {
    const { width, height, data } = imageData;
    const n = width * height;
    if (n === 0) return;
    ...
    for (let p = 0, i = 0; p < n; p++, i += 4) {
      ...
      cb[p] = -0.169 * r - 0.331 * g + 0.5 * b;
      cr[p] = 0.5 * r - 0.419 * g - 0.081 * b;
    }

    const radius = Math.max(2, Math.round(Math.min(width, height) * 0.02));
    const cbBlur = cb.slice();
    const crBlur = cr.slice();
    boxBlur(cbBlur, width, height, radius, 3);
    boxBlur(crBlur, width, height, radius, 3);

    for (let p = 0, i = 0; p < n; p++, i += 4) {
      const r = data[i], g = data[i + 1], b = data[i + 2];
      const y = 0.299 * r + 0.587 * g + 0.114 * b;
      const newCb = cb[p] + (cbBlur[p] - cb[p]) * amount;
      const newCr = cr[p] + (crBlur[p] - cr[p]) * amount;
      data[i] = clamp(y + 1.402 * newCr);
      data[i + 1] = clamp(y - 0.344 * newCb - 0.714 * newCr);
      data[i + 2] = clamp(y + 1.772 * newCb);
    }
  }
  ...
  /**
   * Grain / bruit : un bruit indépendant par pixel a un défaut caché — sa
   * taille apparente dépend de la résolution. Sur l'aperçu (réduit), chaque
   * grain occupe une part relativement grande de l'image et se voit bien ;
   * une fois exporté en pleine résolution (et donc sauvegardé dans la
   * galerie du téléphone), le même bruit devient minuscule et quasi
   * invisible. On génère donc le bruit sur une grille dont la taille de
   * cellule est proportionnelle aux dimensions de l'image, puis on
   * l'interpole en douceur (comme pour la halation) : le grain garde une
   * taille apparente cohérente quelle que soit la résolution d'export, et
   * ses amas organiques ressemblent davantage à un vrai grain de pellicule
   * qu'à du bruit numérique uniforme.
   */
  function grain(imageData, amount) {
    const { width, height, data } = imageData;
    const rand = fastRandom(42);

    const cellSize = Math.max(1, Math.round(Math.min(width, height) / 420));
    ...
    const noiseB = new Float32Array(n);
    for (let i = 0; i < n; i++) {
      noiseL[i] = rand() - 0.5;
      noiseR[i] = rand() - 0.5;
      noiseB[i] = rand() - 0.5;
    ...
    const intensity = amount * 175;
    const colorBleed = amount * 34; // bruit chromatique plus marqué, façon pellicule couleur

    for (let y = 0; y < height; y++) {
      const fy = y / cellSize;
      const gy0 = Math.min(gh - 2, Math.floor(fy));
      const gy1 = gy0 + 1;
      const wy = fy - gy0;
      const rowBase = y * width * 4;
      for (let x = 0; x < width; x++) {
        const fx = x / cellSize;
        const gx0 = Math.min(gw - 2, Math.floor(fx));
        const gx1 = gx0 + 1;
        const wx = fx - gx0;
        const i00 = gy0 * gw + gx0, i10 = gy0 * gw + gx1, i01 = gy1 * gw + gx0, i11 = gy1 * gw + gx1;

        const lumN = bilerp(noiseL, i00, i10, i01, i11, wx, wy) * intensity;
        const crN = bilerp(noiseR, i00, i10, i01, i11, wx, wy) * colorBleed;
        const cbN = bilerp(noiseB, i00, i10, i01, i11, wx, wy) * colorBleed;

        const idx = rowBase + x * 4;
        data[idx] = clamp(data[idx] + lumN + crN);
        data[idx + 1] = clamp(data[idx + 1] + lumN);
        data[idx + 2] = clamp(data[idx + 2] + lumN + cbN);
      }
    }
  }
